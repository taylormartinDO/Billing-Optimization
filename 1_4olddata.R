...
#Read tabs
tab1 <- read_excel("/Users/taylormartin/Desktop/statistics/olddata.xlsx", sheet = "no CPT changes")
tab2 <- read_excel("/Users/taylormartin/Desktop/statistics/olddata.xlsx", sheet = "yes CPT changes")
...
plot_data <- data.frame(
  Cohort = c("SDE", "No SDE"),
  Proportion = c(proportion_sde, proportion_no_sde)
)
...
ggplot(plot_data, aes(x = Cohort, y = Proportion, fill = Cohort)) +
  ...
  labs(title = "Proportion of Successes in SDE and No SDE Cohorts",
       x = "Cohort",
       y = "Proportion") +
  theme_minimal()
